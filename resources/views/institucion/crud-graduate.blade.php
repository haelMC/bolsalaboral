        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            <div class="flex items-center justify-between">
                ...
                <div class="lg:ml-40 ml-10 space-x-8">
                    ...
                    @if ($isOpen)
                        @include('institucion.graduate-create')
                    @endif
                </div>
            </div>
                ...
                <table class="w-full divide-y divide-gray-200 table-auto">
                    <thead class="bg-indigo-900 text-white">
                        <tr class="text-left text-xs font-bold  uppercase">
                            ...
                            <td scope="col" class="px-6 py-3">Numero de Contacto</td>
                            <td scope="col" class="px-6 py-3">especialidad</td>
                            <td scope="col" class="px-6 py-3">nivel academico</td>
                            <td scope="col" class="px-6 py-3">institucion</td>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($graduates as $item)
                            <tr class="text-sm font-medium text-gray-900">
                                ...
                                <td class="px-6 py-4">{{ $item->academic_level }}</td>
                                <td class="px-6 py-4">
                                    @php
                                        $institution = App\Models\Institution::find($item->institution_id);
                                    @endphp
                                    {{ $institution ? $institution->name : 'Usuario no encontrado' }}
                                </td>
            ...
            Livewire.on('deleteItem', id => {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        //alert("del");
                        Livewire.emitTo('crud-graduate', 'delete', id);
                        Swal.fire(
                            'Deleted!',
