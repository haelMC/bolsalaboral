        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            <div class="flex items-center justify-between">
                ...
                <div class="lg:ml-40 ml-10 space-x-8">
                    <button wire:click="create()" class="bg-indigo-900 px-4 py-2 rounded-md text-white font-semibold tracking-wide cursor-pointer">
                    ...
                    <tbody class="divide-y divide-indigo-900">
                        @foreach ($monitorings as $item)
                            <tr class="text-sm font-medium text-gray-900">
                                <td class="px-6 py-4">
                                    <!-- Imagen y datos del docente y supervisado -->
                                    <div class="flex mb-5 -space-x-4">
                                        ...
                                        @isset($item->graduate)
                                            <div class="relative h-10 w-10">
                                                <img class="w-full h-full object-cover border-gray-400 rounded-full border-2 border-white"
                                ...
                                <td class="px-6 py-4">{{ $item->teacher->user->full_name ?? 'Teacher not found' }}</td>
                                <td class="px-6 py-4">{{ $item->graduate->user->full_name ?? 'Egresado no encontrado' }}</td>
                                <td class="px-6 py-4">{{ $item->created_at }}</td>
                                <td class="px-6 py-4">{{ $item->updated_at }}</td>

                                <td class="px-6 py-4">
                                    <!-- Opciones de editar y eliminar solo visibles para admin -->
                                    @role('admin')
